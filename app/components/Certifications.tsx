        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {certifications.map((cert, index) => (
            <ScrollReveal key={index} delay={index * 100}>
              <TiltCard
                className={`card card-hover p-6 text-center relative overflow-hidden h-full ${cert.status === "in-progress" ? "opacity-80" : ""
                  }`}
              >
                {/* Badge glow effect */}
                <div
                ...
                <p
                  className={`text-sm font-medium ${cert.status === "earned" ? "text-primary" : "text-gray-500"
                    }`}
                >
                  {cert.level}
                </p>
